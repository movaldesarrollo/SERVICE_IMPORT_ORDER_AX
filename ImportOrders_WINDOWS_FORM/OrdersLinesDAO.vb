...
#Region "FUNCTIONS"
    ...
    End Function
    'Get configuration
    Public Function getConfig(ByRef importPath As String, ByRef importMinInterval As Integer) As Boolean
        getConfig = False
        Try
            Dim sel As String = "Select importsMinutsInterval, importFilePath from master where numYear = 2022"
    ...
    Public Sub updateImportServices(id As Integer, text As String)
        con.Open()
        Try
            Dim insertQuery As String = "update imports_services set logText = '" & Replace(text, "'", "|") & "' where id = " & id
            Dim cmd As New SqlCommand(insertQuery, con)
            cmd.ExecuteNonQuery()
        Catch ex As Exception
        End Try
...
SELECT ar.id,ar.name,ar.monoSKU ,aac.name cname FROM articles ar
left join articles_articles_clients aac on aac.id_articles = ar.id 
where ar.name =  order by aac.name desc"
